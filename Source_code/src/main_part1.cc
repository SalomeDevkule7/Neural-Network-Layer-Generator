...
void genDatapath(int M, int N, int bits, ofstream &os, string romModName_W, string romModName_B);
void genControlPath(int M, int N, int bits, ofstream &os);
...
void genLayer(int M, int N, int P, int bits, vector<int>& constVector, string modName, ofstream &os) {
   ...
   // actual numbers
   int addrBits_W = ceil(log2(M*N));
   int addrBits_B = ceil(log2(M));
   ...
   os << "   logic accum_src, en;" << endl;
   os << "   logic wr_en_a;" << endl;
   os << "   logic wr_en_m;" << endl;
   os << "   logic wr_en_x;" << endl;
   os << "   logic [" << addrBits_W+1 << ":0] addr_W;" << endl;
   os << "   logic [" << addrBits_B+1 << ":0] addr_B;" << endl;
   os << "   logic [" << addrBits_X << ":0] addr_X;" << endl;
   os << "   mvm_controlpath cp(clk, s_valid, reset, m_ready, addr_W,addr_X, wr_en_x, m_valid,s_ready,accum_src,en,addr_B);" << endl;
   os << "   mvm_datapath datapath(wr_en_x,addr_X,data_in,data_out,clk,reset,en,accum_src,addr_W,addr_B);" << endl;
   ...
   genDatapath(M, N, bits, os, romModName_W, romModName_B);
   
   //Generate control path module
   genControlPath(M, N, bits, os);
...
void genDatapath(int M, int N, int bits, ofstream &os, string romModName_W, string romModName_B) {
      ...
      int addrBits_B = ceil(log2(M));
      int addrBits_X = ceil(log2(N));
      os << "module mvm_datapath(wr_en_x,addr_X,data_in,data_out,clk,reset,en,accum_src,addr_W,addr_B);" << endl;
      os << "   input wr_en_x,clk,reset;" << endl;
      os << "   input [" << addrBits_W+1 << ":0] addr_W;" << endl;
      ...
      os << "   input en;" << endl;
      os << "   output logic signed [" << bits-1 << ":0] data_out;" << endl;
      os << "   logic signed [" << bits-1 << ":0] adder_data_out;" << endl;
      ...
      os << "//*************************************************************" << endl;
      os << "   logic signed [" << bits-1 << ":0] out_rom_W;" << endl;
      os << "   logic signed [" << bits-1 << ":0] out_rom_B;" << endl;
      os << "   logic signed [" << bits-1 << ":0] out_rom_r;" << endl;
      os << "//****************PIPELINING***********************************" << endl;
      ...
      os << "//***************ROM MODULE FOR WEIGHTS*************************" << endl;
      os << "   " << romModName_W << " rom_W(.clk(clk),.addr(addr_W),.z(out_rom_W));" << endl;
      os << "//***************ROM MODULE FOR BIAS****************************" << endl;
      os << "   " << romModName_B << " rom_B(.clk(clk),.addr(addr_B),.z(out_rom_B));" << endl << endl;
      os << "//********************Memory x**********************************" << endl;
...
void genControlPath(int M, int N, int bits, ofstream &os) {
      ...
      int addrBits_B = ceil(log2(M));
      int addrBits_X = ceil(log2(N));
      os << "module mvm_controlpath(clk, s_valid, reset, m_ready, addr_W,addr_X, wr_en_x, m_valid,s_ready,accum_src,en,addr_B);" << endl;
      os << "   input clk, s_valid, reset, m_ready;" << endl;
      os << "   output logic s_ready, m_valid;" << endl;
      os << "   output logic wr_en_x;" << endl;
      os << "   output logic[" << addrBits_W+1 << ":0] addr_W;" << endl;
